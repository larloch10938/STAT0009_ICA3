...
alpha <- function(j,i){
m=min( c( h(j)/h(i), 1 ))  #comparison of the proposal distribution ratio and 1
m
}

metro <- function(n){ #the metropolis-hastings algorithm 
  x=10
  for(k in 1:n){
    i = x[length(x)]
    j = Yq(i)
...
z = replicate(1000, metro(1000)) #run the algorithm 1000 times for precision

hist(z, prob=TRUE, breaks=100, to = 30) #plot of the distribution as histograms
x= seq(0, 1, by =0.1)
