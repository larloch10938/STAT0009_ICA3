...
Yq <- function(x){      #Yq function calculates the proposal distribution for each x
   y= 1 + rgeom(1,x)    #As we are dealing with a discrete distribution, a good idea would be to use a discrete distribution for our proposed distribution Q. 
 y                      #Here, we shall use the geometric distribution as it only outputs positive integers and can help us explore higher states
                        #than our initial state due to its unboundness.
}
...
hist(n, prob=TRUE, breaks=100, xlim=c(0,10), xlab = "n", ylab= expression(mu(n)),  main="Sample distribution of Mu")  #plot of the distribution as probability histogram
...
##2.3
In the previous expression we can identify $\theta^{-(\alpha+n)-1}\exp[-\tfrac{\beta+t(x)}{\theta }]$ as the kernel of and Inverse Gamma distributions. Therefore, $s(\theta|t(x))$ follows an Inverse Gamma distribution with parameters $\alpha +n$ and $\beta + t(x)$. Therefore, the proportionality constant, C, must be such that $$C = \frac{[\beta + t(x)]^{\alpha+n}}{\Gamma(\alpha +n)}.$$
...
#Adding the triangle and the central angle.
...
set.seed(NULL)
```
Now the explanation of this Poisson process, $\Upsilon$, is as follows. If $M=m$, then we have $X_1,X_2,...X_m$ random variables, where $X_i$ can be expressed as $X_i=cos(\alpha_i)$ for $i=1,2,...m$, where $\alpha_i$ is the central angle as defined previously. We know that $\alpha_i$ is a random variable uniformly distributed on $[0,\pi]$, therefore  $f_{\alpha_i}(\alpha_i)= \frac{1}{\pi}$. Then, to express the density of $X_i$, we need to use a transformation. If $X_i=cos(\alpha_i)$, then $\alpha_i=arccos(x_i)$. Then, $f(X_i|M=m)=f_{\alpha_i}(\alpha_i)\times \Big|\frac{d\alpha_i}{dx_i}\Big|$. Since $f_{\alpha_i}(\alpha_i)$ is already known and we can compute $\Big|\frac{d\alpha_i}{dx_i}\Big|$ as $\frac{1}{\sqrt{1-x_i^2}}$, $f(X_i|M=m)=\frac{1}{\pi\sqrt{1-x_i^2}}$. This is clearly not a pdf of a uniform distribution as it depends on the value of each $x_i$. Therefore, $\Upsilon$ cannot be a homogeneous Poisson process.
