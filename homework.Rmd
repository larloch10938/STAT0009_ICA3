...
opt2 <- function(t,n){
  ...
  while (item[j] == 0){service1 = rexp(L, 4) ## queue of red items
  ...
  while (item[j] == 1){service2 = rexp(L, 4) ## queue of blue items
...
y= replicate(10, opt2(600,10))
y1 = y[1,]
y2 = y[2,]
b1 = seq(-1,max(y1)+1, by=1)
b2 = seq(-1,max(y2)+1, by=1)
hist(y1, prob=TRUE, breaks=b1)
