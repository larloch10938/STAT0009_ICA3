...
pi <- function(n){ #pi function is the target distribution we have up to a constant
=======
pi_function <- function(x){ #h function is the target distribution we have up to a constant
>>>>>>> 85066d879417088734877d4db9e4ecb0cc7acfbb
 z=0
...
alpha <- function(j,i){ #alpha function works out the probability of moving from state i to state j
  ...
  m=min( c( pi(j)/pi(i)*dgeom(i-1, prob_j)/dgeom(j-1, prob_i), 1)) 
  m          
=======
alpha <- function(j,i){ #alpha function works out the probability of moving from state i to  state j
m=min( c( pi_function(j)/pi_function(i), 1 ))  #comparison of the proposal distribution ratio and 1
m
>>>>>>> 85066d879417088734877d4db9e4ecb0cc7acfbb
}
...
plot(points, col="red", xlab = "n", ylab= expression(mu(n)),  main="Sample distribution of Mu") #the actual distribution as verification with Apery's constant
hist(n, prob=TRUE, breaks=500, xlim=c(0,10), add=TRUE) #plot of the distribution as probability histogram
legend("topright", legend=c("Apery's constant distribution", "Sampling from Metropolis-Hastings"), col=c("gray", "red"), lty=c(1,1))
...
d= replicate(1000, metro(1000))
t <-table(d) 
print(t)
approx = 1/(t[1]/1000^2* 1^3)
approx
```
...
z = replicate(1000, metro(1000))  #create 1000 samples

#this is a histogram of independent samples from the target distribution 
#(using the MH-algotrithm)
hist(z, prob=TRUE, breaks=50, xlab = expression(theta),ylab="Probability", main="Posterior distribution of theta", ylim=c(0,0.4), xlim=c(0,50))
x= seq(0, 1, by =0.1)

#curve of the Inverse Gamma (5,15) distribution (this is the true distribution #based on section 2.3)
curve(15^5/24*x^(-6)*exp(-15/x), add=T, col="red")
legend("center",legend=c("Inverse Gamma (5,15)", "Samples using MH"),col=c("gray","red"),lty=c(1,1))
#The curve and the histogram follows roughly the same shape indicating that 
#the Metropolis-Hastings algorithm was indeed succesful and that our guess of
...
testt1  ## find the stationary distribution of the first option
...
y= replicate(500, opt2(600)) ## simulation by large number

a = seq(-1,max(y)+1, by=1)

hist(y, prob=TRUE, breaks=a) ## negative binormial (sum of two geometric distribution)

mean(x)  ## average items in queue in the first option
mean(y)  ## average items in queue in the second option

```
* Compared with mean value of items in two options, the option2 have value around 7, and option1
have value around 3. These value stands for the average number of items in the corresponding 
queues. From the value we can see that option2 have more items in it on average.
