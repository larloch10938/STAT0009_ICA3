...
## code to do here
t = replicate(500, metro(500))
mean = sum(t)/500 # needs to be 1.37 
mean
approx = (pi**2)/(6*mean)
approx
```
...
#The histogram and the plot are hard to interpret.
hist(reps)
plot(reps)
...
nonhom <- function(x,y){     
          ...
          print(c(numpoint1*1/(100*(min(x,y)+1)),
                  numpoint2*1/(100*(abs(x-y))), 
                  numpoint3*1/(100*(1-max(x,y)))))
}
